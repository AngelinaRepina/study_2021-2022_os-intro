...
1. Для выполнения лабораторной работы создала четыре файла с помощью команды touch и откроем их в emacs для редактирования (рис 1)

![1: создание файлов](image/1.png){ #fig:01 width=70% }

2. Изменим код программ, предоставленных в тексте задания лабораторной работы. В файл common.h добавила стандартные заголовочные файлы unisd.h и time.h, которые необходимы для работы других файлов (рис 2). Этот файл предназначен для заголовочных файлов, чтобы не прописывать их в других программах каждый раз

![2: common.h](image/2.png){ #fig:01 width=70% }

Затем в файл server.c добавим цикл while для контроля за временем работы сервера (рис 3, 4, 5)., причем время от начала работы сервера до настоящего не должно превышать 30 секунд
...
В файл client.c добавим цикл for который будет отвечать за количество сообщений о текущем времени (4 сообщения), и команду sleep(5) для остановки работы клиента через 5 секунд. Также я изменила выводимое сообщение на текущее время (рис 6, 7)

![6: client.c 1](image/6.png){ #fig:01 width=70% }

![7: client.c 2](image/7.png){ #fig:01 width=70% }

Makefile оставила без изменений (рис 8)

![8: Makefile](image/8.png){ #fig:01 width=70% }

3. Используя команду make all (рис 9), скомпилировала необходимые для работы файлы

![9: компиляция файлов](image/9.png){ #fig:01 width=70% }

4. Проверим работу написанного кода

Открыла три терминала, в первом окне запустила программу ./server, во втором и третьем ./client. В результате каждый терминал-клиент вывел по четыре сообщения о текущем времени. Спустя 30 секунд работы сервера была прекращена (рис 10)
